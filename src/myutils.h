...
void *rce_malloc(uint64 n, uint64 Bytes, const char *FileName, int Line);
void *rce_malloc64(uint64 n, uint64 Bytes, const char *FileName, int Line);
...
#define mymalloc64(n, m)	rce_malloc64((n), (m), __FILE__, __LINE__)
#define myfree(p)			rce_free(p, __FILE__, __LINE__)
#define myalloc(t, n)		(t *) rce_malloc((n), sizeof(t), __FILE__, __LINE__)
#define myalloc64(t, n)		(t *) rce_malloc((n), sizeof(t), __FILE__, __LINE__)

#else // RCE_MALLOC

void *mymalloc(unsigned n, unsigned bytes);
void *mymalloc64(uint64 n, uint64 bytes);
void myfree(void *p);
...
#define CloseStdioFile(f)	CloseStdioFile_(&f)
...
static inline double GetRatio(double x, double y) { if (y == 0) {return 0; } return x/y; }
...
typedef void (*FN_PROGRESS_CALLBACK)(string &s, double &PctDone);
...
#define ProgressLoopTick(i)	(i%10000 == 0 ? Progress_OnTick(), 0 : 0)

void ProgressOne(const char *szFormat, ...);
...
void ProgressLoop(uint32 Count, uint32 *ptrIndex, const string &Msg = "Processing");
void ProgressLoop64(uint64 Count, uint64 *ptrIndex, const string &Msg = "Processing");
